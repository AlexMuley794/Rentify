...
@section('title', (isset($transaction) ? 'Editar' : 'Nueva') . ' Transacción - Rentify')
@section('page-title', isset($transaction) ? 'Editar Transacción' : 'Nueva Transacción')
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white">
            <i class="fas fa-exchange-alt text-indigo-600 mr-3"></i>{{ isset($transaction) ? 'Editar Transacción' : 'Nueva Transacción' }}
        </h1>
        <p class="text-gray-600 dark:text-gray-400 mt-2">{{ isset($transaction) ? 'Actualiza el registro de la transacción' : 'Registra una nueva transacción (ingreso o gasto)' }}</p>
    </div>

    <!-- Form Card -->
    <div class="bg-white dark:bg-gray-900 rounded-xl shadow-lg border border-gray-200 dark:border-gray-800 overflow-hidden">
        <form action="{{ isset($transaction) ? route('transactions.update', $transaction) : route('transactions.store') }}" method="POST" class="space-y-6 p-8">
            @csrf
            ...
            <div>
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4 flex items-center">
                    <i class="fas fa-home text-indigo-600 mr-3"></i>Selecciona Propiedad
                </h2>
                <div>
            ...
            <div class="border-t border-gray-200 dark:border-gray-700 pt-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="date" id="date" name="date" value="{{ old('date', isset($transaction) ? $transaction->date->format('Y-m-d') : now()->format('Y-m-d')) }}" required class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent shadow-sm transition">
                        @error('date')
                            <p class="text-red-500 text-sm mt-2 flex items-center"><i class="fas fa-exclamation-circle mr-2"></i>{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div class="border-t border-gray-200 dark:border-gray-700 pt-8 flex flex-col sm:flex-row items-center gap-4">
                <button type="submit" class="w-full sm:w-auto bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-700 hover:to-indigo-800 text-white font-semibold py-3 px-8 rounded-xl transition-all duration-200 flex items-center justify-center shadow-lg hover:shadow-xl">
                    <i class="fas {{ isset($transaction) ? 'fa-save' : 'fa-plus' }} mr-2"></i>{{ isset($transaction) ? 'Actualizar Transacción' : 'Crear Transacción' }}
                </button>
