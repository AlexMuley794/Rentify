...
@section('title', (isset($tenant) ? 'Editar' : 'Nuevo') . ' Inquilino - Rentify')
@section('page-title', isset($tenant) ? 'Editar Inquilino' : 'Nuevo Inquilino')
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white">
            <i class="fas fa-user-plus text-indigo-600 mr-3"></i>{{ isset($tenant) ? 'Editar Inquilino' : 'Nuevo Inquilino' }}
        </h1>
        <p class="text-gray-600 dark:text-gray-400 mt-2">{{ isset($tenant) ? 'Actualiza los datos del inquilino' : 'Registra un nuevo inquilino en tu sistema' }}</p>
    </div>

    <!-- Form Card -->
    <div class="bg-white dark:bg-gray-900 rounded-xl shadow-lg border border-gray-200 dark:border-gray-800 overflow-hidden">
        <form action="{{ isset($tenant) ? route('tenants.update', $tenant) : route('tenants.store') }}" method="POST" class="space-y-6 p-8">
            @csrf
            @if(isset($tenant))
                @method('PUT')
            @endif
            ...
            <div class="border-t border-gray-200 dark:border-gray-700 pt-6">
                ...
                <div>
                    ...
                    <textarea id="notes" name="notes" rows="5" class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent shadow-sm transition" placeholder="Añade notas sobre el inquilino: historial de pagos, referencias, restricciones, etc...">{{ old('notes', $tenant->notes ?? '') }}</textarea>
                    @error('notes')
                        <p class="text-red-500 text-sm mt-2 flex items-center"><i class="fas fa-exclamation-circle mr-2"></i>{{ $message }}</p>
                    @enderror
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-2"><i class="fas fa-info-circle mr-1"></i>Máximo 500 caracteres</p>
                </div>
            </div>
            ...
            <div class="border-t border-gray-200 dark:border-gray-700 pt-8 flex flex-col sm:flex-row items-center gap-4">
                <button type="submit" class="w-full sm:w-auto bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-700 hover:to-indigo-800 text-white font-semibold py-3 px-8 rounded-xl transition-all duration-200 flex items-center justify-center shadow-lg hover:shadow-xl">
                    <i class="fas {{ isset($tenant) ? 'fa-save' : 'fa-plus' }} mr-2"></i>{{ isset($tenant) ? 'Actualizar Inquilino' : 'Crear Inquilino' }}
                </button>
