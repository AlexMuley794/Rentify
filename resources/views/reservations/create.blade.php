...
@section('title', (isset($reservation) ? 'Editar' : 'Nueva') . ' Reserva - Rentify')
@section('page-title', isset($reservation) ? 'Editar Reserva' : 'Nueva Reserva')
...
<div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white">
            <i class="fas fa-calendar-check text-indigo-600 mr-3"></i>{{ isset($reservation) ? 'Editar Reserva' : 'Nueva Reserva' }}
        </h1>
        <p class="text-gray-600 dark:text-gray-400 mt-2">{{ isset($reservation) ? 'Actualiza los detalles de la reserva' : 'Crea una nueva reserva para una propiedad' }}</p>
    </div>

    <!-- Form Card -->
    <div class="bg-white dark:bg-gray-900 rounded-xl shadow-lg border border-gray-200 dark:border-gray-800 overflow-hidden">
        <form action="{{ isset($reservation) ? route('reservations.update', $reservation) : route('reservations.store') }}" method="POST" class="space-y-6 p-8">
            @csrf
            ...
            <div>
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4 flex items-center">
                    <i class="fas fa-link text-indigo-600 mr-3"></i>Selecciona Propiedad e Inquilino
                </h2>
                        ...
                        <select id="property_id" name="property_id" required class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent shadow-sm transition">
                            ...
                            @foreach($properties as $property)
                                <option value="{{ $property->id }}" {{ old('property_id', $reservation->property_id ?? '') == $property->id ? 'selected' : '' }}>
                                    🏠 {{ $property->title }} - €{{ number_format($property->price, 2, ',', '.') }}/mes
                                </option>
                            @endforeach
                        </select>
                        @error('property_id')
            ...
            <div class="border-t border-gray-200 dark:border-gray-700 pt-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <input type="date" id="start_date" name="start_date" value="{{ old('start_date', isset($reservation) ? $reservation->start_date->format('Y-m-d') : '') }}" required class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent shadow-sm transition">
                        @error('start_date')
                            <p class="text-red-500 text-sm mt-2 flex items-center"><i class="fas fa-exclamation-circle mr-2"></i>{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <input type="date" id="end_date" name="end_date" value="{{ old('end_date', isset($reservation) ? $reservation->end_date->format('Y-m-d') : '') }}" required class="w-full px-4 py-3 border border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-white rounded-xl focus:ring-2 focus:ring-indigo-500 focus:border-transparent shadow-sm transition">
                        @error('end_date')
                            <p class="text-red-500 text-sm mt-2 flex items-center"><i class="fas fa-exclamation-circle mr-2"></i>{{ $message }}</p>
                        @enderror
                    </div>
            ...
            <div class="border-t border-gray-200 dark:border-gray-700 pt-6">
                ...
                <div>
                    ...
                    @error('total_price')
                        <p class="text-red-500 text-sm mt-2 flex items-center"><i class="fas fa-exclamation-circle mr-2"></i>{{ $message }}</p>
                    @enderror
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-2"><i class="fas fa-info-circle mr-1"></i>Introduce el precio total por el período de alquiler</p>
                </div>
            </div>
            ...
            <div class="border-t border-gray-200 dark:border-gray-700 pt-8 flex flex-col sm:flex-row items-center gap-4">
                <button type="submit" class="w-full sm:w-auto bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-700 hover:to-indigo-800 text-white font-semibold py-3 px-8 rounded-xl transition-all duration-200 flex items-center justify-center shadow-lg hover:shadow-xl">
                    <i class="fas {{ isset($reservation) ? 'fa-save' : 'fa-plus' }} mr-2"></i>{{ isset($reservation) ? 'Actualizar Reserva' : 'Crear Reserva' }}
                </button>
