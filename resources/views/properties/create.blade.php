...
@section('title', (isset($property) ? 'Editar' : 'Nueva') . ' Propiedad - Rentify')
@section('page-title', isset($property) ? 'Editar Propiedad' : 'Nueva Propiedad')
...
<div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="mb-8">
        <h1 class="text-3xl font-bold text-gray-900 dark:text-white">
            <i class="fas fa-building text-indigo-600 mr-3"></i>{{ isset($property) ? 'Editar Propiedad' : 'Nueva Propiedad' }}
        </h1>
        <p class="text-gray-600 dark:text-gray-400 mt-2">{{ isset($property) ? 'Actualiza los detalles de tu propiedad' : 'Crea una nueva propiedad en tu portafolio' }}</p>
    </div>

    <!-- Form Card -->
    <div class="bg-white dark:bg-gray-900 rounded-xl shadow-lg border border-gray-200 dark:border-gray-800 overflow-hidden">
        <form action="{{ isset($property) ? route('properties.update', $property) : route('properties.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6 p-8">
            @csrf
            @if(isset($property))
                @method('PUT')
            @endif
            ...
            <div class="border-t border-gray-200 dark:border-gray-700 pt-6">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white mb-4 flex items-center">
                    <i class="fas fa-details text-indigo-600 mr-3"></i>Detalles de la Propiedad
                </h2>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        @error('image_path')
                            <p class="text-red-500 text-sm mt-2 flex items-center"><i class="fas fa-exclamation-circle mr-2"></i>{{ $message }}</p>
                        @enderror
                        @if(isset($property) && $property->image_path)
            ...
            <div class="border-t border-gray-200 dark:border-gray-700 pt-8 flex flex-col sm:flex-row items-center gap-4">
                <button type="submit" class="w-full sm:w-auto bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-700 hover:to-indigo-800 text-white font-semibold py-3 px-8 rounded-xl transition-all duration-200 flex items-center justify-center shadow-lg hover:shadow-xl">
                    <i class="fas {{ isset($property) ? 'fa-save' : 'fa-plus' }} mr-2"></i>{{ isset($property) ? 'Actualizar Propiedad' : 'Crear Propiedad' }}
                </button>
